...
class HBCIDataElementDescription: HBCISyntaxElementDescription {
    ...
    override init(syntax: HBCISyntax, element: XMLElement) throws {
        ...
        if let type = self.type {
            if(!setDataType(type)) {
                logInfo("Syntax file error: unknown data type \(self.type!)");
                throw HBCIError.syntaxFileError;
            }
        } else {
            logInfo("Syntax file error: Data Element has no type");
            throw HBCIError.syntaxFileError;
        }
    ...
    override func parse(_ bytes: UnsafePointer<CChar>, length: Int, binaries:Array<Data>) ->HBCISyntaxElement? {
        ...
        if sidx > 0 && tidx > 0 {
            if let sValue = String(cString: target, encoding: String.Encoding.isoLatin1) {
                ...
                if let type = self.dataType {
                    ...
                    case .value:
                        if let value = _numberFormatter.number(from: sValue) {
                            ...
                            logInfo("Parse error: string \(sValue) cannot be converted to a number");
                            return nil;
                        }
                    ...
                    case .date:
                        if let date = _dateFormatter.date(from: sValue) {
                            ...
                            logInfo("Parse error: string \(sValue) cannot be converted to a date");
                            return nil;
                        }
                    case .time:
                        if let time = _timeFormatter.date(from: sValue) {
                            ...
                            logInfo("Parse error: string \(sValue) cannot be converted to a time");
                            return nil;
                        }
                    case .binary:
                        if sValue.hasPrefix("@") && sValue.hasSuffix("@") {
                            ...
                            logInfo("Invalid binary tag: \(sValue)");
                            return nil;
                        }
                    case .numeric:
                        de.value = Int(sValue);
                ...
                logInfo("Parse error: data cannot be converted to String");
                return nil;
            }
        } 
